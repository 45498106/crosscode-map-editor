...
import { TilemapDrawer } from '../shared/phaser/tilemap/tilemap-drawer';

@Component({
	selector: 'app-phaser-tilemap',
	templateUrl: './phaser-tilemap.component.html',
	styleUrls: ['./phaser-tilemap.component.scss']
})
export class PhaserTilemapComponent implements OnInit, OnDestroy {
	game: Phaser.Game;
	// tileMap: CCMap;
	sub: Subscription;
	...
	private tileDrawer: TilemapDrawer;
	...
	ngOnInit() {
		this.http.getAllFiles().subscribe(res => {
			this.game = new Phaser.Game(screen.width * window.devicePixelRatio, screen.height * window.devicePixelRatio,
				Phaser.CANVAS, 'content-tilemap', {
					create: () => this.create(),
					update: () => this.update(),
					render: () => this.render(),
	...
	create() {
		...
		this.tileDrawer = game.plugins.add(TilemapDrawer);
		...
		this.mapLoader.selectedLayer.subscribe(layer => {
			if (layer) {
				const size = layer.getFullTilesetSize();
				this.windowWidth = size.x + 'px';
				this.windowHeight = size.y + 'px';
				return this.tileDrawer.selectLayer(layer);
			} else {
				this.windowWidth = '300px';
				this.windowHeight = '0';
			}
